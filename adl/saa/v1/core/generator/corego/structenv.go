...
func AddParseEnvFunc(fieldPrefix string, node *ast.Struct) (*ast.Func, error) {
	...
	for _, field := range node.Fields() {
		...
		stereotype.SetEnvName(field, flagName)
		...
		case *ast.SimpleTypeDecl:
			...
			default:
				return fun, core.NewPosError(core.NewNodeFromAst(field), field.FieldName+" "+field.FieldType.String()+": unsupported field type for struct parse env function")
			}
